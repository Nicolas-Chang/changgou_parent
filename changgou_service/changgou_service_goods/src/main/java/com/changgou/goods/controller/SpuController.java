...
public class SpuController {
    ...
    public Result findGoodsById(@PathVariable("id") String id){
        Goods goods = spuService.fingGoodsById(id);
    ...
    public Result audit(@PathVariable("id") String id){
        spuService.audit(id);
        return new Result(true,StatusCode.OK,"修改审核状态完成,已完成自动上架");
    }

    @PutMapping("/pull/{id}")
    public Result pull(@PathVariable("id") String id){
        spuService.pull(id);
        return new Result(true,StatusCode.OK,"下架");
    }
    ...
    public Result deleted(@PathVariable("id") String id){
        spuService.deleyed(id);
        return new Result(true,StatusCode.OK,"逻辑删除");
    }
    @GetMapping ("select/name")
    public Result function12(@RequestParam Map map){
        List<Spu> selected = spuService.selected(map);
        return new Result(true,StatusCode.OK,"s",selected);
    }
}
