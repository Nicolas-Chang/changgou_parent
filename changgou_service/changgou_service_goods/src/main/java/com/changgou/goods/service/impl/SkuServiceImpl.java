...
public class SkuServiceImpl implements SkuService {
    ...
    public void update(String username) {
        ...
        for (OrderItem orderItem : orderItems) {
            int num = skuMapper.updateSkuNum(orderItem);
            if(num <= 0){
    ...
    @Override
    public int resumeStockNum(String skuId, Integer num) {
        int i = skuMapper.resumeStockNum(skuId,num);
        if(i < 1){
            throw new RuntimeException("复原失败");
        }
        return i ;
    }
