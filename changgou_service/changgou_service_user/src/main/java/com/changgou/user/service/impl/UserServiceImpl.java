...
import com.changgou.user.dao.PointLoginMapper;
...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    private PointLoginMapper pointLoginMapper;
    ...
    @Override
    public int updateOptions(Integer money, String username) {
        int points = userMapper.updatePoints(money, username);
        if(points <= 0){
    ...
    @Override
    @Transactional
    public int uodateOptines(Task task) {
        //开始处理业务
        ...
        PointLog pointLog = pointLoginMapper.findPointLoginByOrderId(orderId);
        if(pointLog != null){
            return 0;
        }
        //j将任务存入Redis中去
        redisTemplate.boundValueOps(task.getId()).set("exixt",1, TimeUnit.MINUTES);

        //修改用户积分
        Integer money = point;
        userMapper.updatePoints(money,username);
        ...
        int i = pointLoginMapper.insertSelective(pointLog);
        if(i <= 0){
            return i;
        }
